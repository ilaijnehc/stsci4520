...
plot_storm_tracks <- function(hurdat, storms) {
  ...
  # plot the storm tracks on top of the map
  ggplot() +
    geom_path(data = hurdat_selected, aes(x = longitude, y = latitude, group = name, color = name)) +
    geom_path(data = us_map_data, aes(x = long, y = lat, group = group), color = "grey50") +
    geom_path(data = map_data, aes(x = long, y = lat, group = group), color = "grey50") +
    coord_map() +
